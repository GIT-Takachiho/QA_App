...
import jp.techacademy.takaomi.okabe.qa_app.R.string.app_name
...
class MainActivity : AppCompatActivity(), NavigationView.OnNavigationItemSelectedListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // ナビゲーションドロワーの設定
        val toggle = ActionBarDrawerToggle(this, drawer_layout, toolbar, app_name, app_name)
    ...
    override fun onResume() {
        ...
        val user = FirebaseAuth.getInstance().currentUser
        val nav_favorite = nav_view.menu.findItem(R.id.nav_favorite)

        nav_favorite.isVisible = user != null
/*
//      上記と同じ
        if (user == null) {
            nav_favorite.isVisible = false
        }else{
            nav_favorite.isVisible = true
        }
 */
/*      又は(動作は同じ　kotlinではあまり記載しない方法)
        if (user == null) {
 ...
 */
    ...
    override fun onNavigationItemSelected(item: MenuItem): Boolean {
        ...
        if(mGenre == 5){
            val intent = Intent(applicationContext, Favorite::class.java)
            startActivity(intent)
            return true
        }else {
